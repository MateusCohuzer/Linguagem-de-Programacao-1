...
class Processamento {
    ...
    public void calcularImcParaTodasAsPessoas() {
        double IMC;
        Pessoa people;
        for (int i = 0; i < listPessoa.size(); i++) {
            people = listPessoa.get(i);
            IMC = people.getPeso() / (people.getAltura() * people.getAltura());
            listPessoa.get(i).setImc(IMC);
            if (IMC < 18.5) {
                listPessoa.get(i).setSituacaoImc("Abaixo do Peso");
            } else if (IMC < 25) {
                listPessoa.get(i).setSituacaoImc("Peso Normal");
            } else if (IMC < 30) {
                listPessoa.get(i).setSituacaoImc("Acima do Peso");
            } else if (IMC < 35) {
                listPessoa.get(i).setSituacaoImc("Obesidade Grau I");
            } else if (IMC < 40) {
                listPessoa.get(i).setSituacaoImc("Obesidade Grau II");
            } else if (IMC >= 40) {
                listPessoa.get(i).setSituacaoImc("Obesidade Grau III/Mórbida");
            }
        }
    }
